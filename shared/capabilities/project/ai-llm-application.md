...
| **S4** Verify (eval harness — mocked / lane-model / opt-in live-model) | `test-engineer` | `DeepSeek API` (codex · yes · metered), `xAI API` (codex · yes · metered) | `eval-harness-pattern` (stub), `representative-workload-design` (stub) | opt-in metered (`default=false`, guarded) + `credential_change` for the live-model provider key |
