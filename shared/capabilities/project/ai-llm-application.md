...
| **S1** Frame (requirements + research) | `product-manager`, `ai-engineer` | `codex --search` (codex · yes · subscription) | `requirements-elicitation` (authored), `scope-decomposition` (authored) | — |
| **S2** Design (agent/RAG arch + eval criteria) | `architect`, `ai-engineer`, `prompt-engineer` | `sequential-thinking` (all · yes · subscription), `context7` (claude · lane-live · subscription) | `dependency-cycle-audit` (authored) | — |
