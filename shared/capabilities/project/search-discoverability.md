...
| **S4** Verify (audit conformance + truth) | `growth-and-search-analyst`, `skeptic`, `content-verifier` | `Google Search grounding` (gemini · yes · subscription) | `technical-seo-audit` (authored) | truth-rights overlay — Rule-8 truth gate (published on-page/schema claims grounded; unverifiable ⇒ `needs_tool`, not PASS); measured ranking/traffic impact is `needs_tool` (no analytics connector) |
