...
| **S1** Frame (release/reliability objectives) | `product-manager`, `site-reliability-engineer` | — | `requirements-elicitation` (authored) | — |
| **S2** Design (pipeline / IaC architecture) | `architect`, `devops-engineer`, `site-reliability-engineer` | `context7` (claude · lane-live · subscription) | `dependency-cycle-audit` (authored), `secret-rotation-discipline` (stub) | `credential_change` (secrets design) |
