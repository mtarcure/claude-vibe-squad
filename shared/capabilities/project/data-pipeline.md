...
| **S1** Frame (data contract) | `product-manager`, `data-extraction-engineer` | — | `requirements-elicitation` (authored), `schema-inference` (stub) | privacy overlay if PII |
| **S2** Design (pipeline architecture) | `architect`, `backend-engineer`, `database-engineer` | `context7` (claude · lane-live · subscription) | `dependency-cycle-audit` (authored) | — |
