...
| **S1** Frame (requirements) | `product-manager`, `architect` | — | `requirements-elicitation` (authored), `scope-decomposition` (authored) | — |
| **S2** Design (API + schema contract) | `architect`, `backend-engineer`, `database-engineer` | `context7` (claude · lane-live · subscription) | `dependency-cycle-audit` (authored) | privacy overlay if PII |
