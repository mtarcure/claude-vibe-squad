...
| **S1** Frame (spec) | `product-manager`, `architect` | — | `requirements-elicitation` (authored), `scope-decomposition` (authored) | — |
| **S2** Design (contract arch + invariants) | `architect`, `smart-contract-engineer` | `context7` (claude · lane-live · subscription) | `dependency-cycle-audit` (authored), `gas-optimization-pattern` (stub) | — |
