...
state_reason: Campaign copy authoring (landing/product/blog + social variants) is text work backed by live tools — `firecrawl` (competitor/reference scrape, claude·lane-live) and `chrono-research-arsenal` (all·lane-live). No catalog-absent tool is load-bearing. The live scope is copy CREATION; the actual multi-channel SEND is `needs_tool`/operator-gated (Gmail `partial`), not claimed live.
state_evidence: registry rows — firecrawl = `claude·lane-live·metered`, chrono-research-arsenal = `all·lane-live·subscription`, chrono-vault = `all·yes·subscription`. Gmail = `claude·partial` (→ send is `needs_tool`, not cited on the core path).
