...
- Lead's own `memory.md` — for distilled domain knowledge that already answers the question
...
**Verification (read-only health check)**: `bin/browser-keep-alive.sh` runs nightly and surfaces missing platform tabs in the morning brief. Lead-level pre-flight for authenticated browser work should use the same raw-CDP `/json/list` check.
...
Per-Lead `memory.md` adds domain-specific overrides on top. The cite at the head of each `memory.md` is mandatory.
...
- Update Lead's `current.md` to clear Active Tasks for this mode
