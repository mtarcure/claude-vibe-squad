...
status: authored
description: Use before implementing against an interface, schema, file format, or contract you did not author — enumerate the assumptions a wrong guess would make expensive (units, nullability, ordering, error paths, idempotency) and pin each with evidence or surface it as a question.
---
