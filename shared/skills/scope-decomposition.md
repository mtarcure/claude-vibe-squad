...
status: authored
description: Break an ambiguous or large ask into bounded, independently verifiable units with explicit scope boundaries before any unit starts.
---
...
5. Bind each unit to an explicit write set: the files it may touch. Overlapping write sets between units are a hidden ordering dependency — serialize them, merge them, or re-cut the boundary so the overlap disappears. This is what a packet's `write_scope` enforces mechanically.
