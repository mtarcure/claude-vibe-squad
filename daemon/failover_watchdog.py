...
def _default_dispatcher(packet: Path, _attempt: dict) -> bool:
    # Route failover redispatch through send-task.sh so it inherits the active
    # SQUAD_DISPATCH_MODE (board by default post-cutover) instead of forcing the
    # legacy pane rail via nudge-task.sh. Prevents reintroducing the double-path
    # the moment failover is enabled.
    completed = subprocess.run(
        ["bash", str(REPO / "bin" / "send-task.sh"), str(packet)],
        cwd=REPO,
