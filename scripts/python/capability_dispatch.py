...
from validate_capabilities import (
    MODES,
    Validator,
    parse_frontmatter,
...
def resolve_snapshot(
    ...
    # (e.g. mode=content capability=content/image) to its canonical folded project
    # id BEFORE validating, so retired-mode packets and links keep dispatching.
    # Non-legacy pointers pass through unchanged. The re-anchored card carries the
    # same gates, so this canonicalizes the home without changing meaning.
    mode, reference, _aliased = resolve_capability_alias(mode, reference)
...
def main() -> int:
    ...
    try:
        try:
            _canonical_mode, canonical_ref, aliased = resolve_capability_alias(
                args.mode, args.capability
            )
        except ValueError as exc:
            raise CapabilityDispatchError(str(exc)) from exc
        if aliased:
            print(
                f"note: legacy capability id {args.capability!r} (mode {args.mode!r}) "
                f"resolved to canonical {canonical_ref!r}",
                file=sys.stderr,
            )
