...
class SubswarmCapacityTests(unittest.TestCase):
    def test_runtime_policy_path_is_repository_relative(self) -> None:
        env = dict(os.environ)
        ...
        result = subprocess.run(
            [
                ...
                (
                    ...
                    "import swarm_runtime; "
                    "print(swarm_runtime.POLICY_PATH)"
                ),
            ],
            cwd=ROOT,
            env=env,
        ...
        self.assertEqual(
            Path(result.stdout.strip()), ROOT / "shared" / "worker-pool-policy.md"
        )
