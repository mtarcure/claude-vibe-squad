...
- **Evidence-gate:** an attack I design is a lead until reproduced (offline) to **≥0.85 confidence** (`multi-agent-evidence-gating`) and settled cross-family, then handed to `impact-validator` for the G1–G4 gate. The moat is beyond commodity: novel attack-chain design and custom probe corpora, not off-the-shelf jailbreak lists.
