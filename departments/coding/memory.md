...
- GPT/Codex model lane `workspace-write` can write under `departments/coding/` but not vault-root `_state/`; draft-only tasks requesting `_state/<name>/...` may need outputs staged under `departments/coding/_state/` and explicitly reported for Chrono relocation.
...
- `codex features list` shows local Codex feature maturity/enabled state. On 2026-05-03 in the GPT/Codex lane: `multi_agent stable true`, `multi_agent_v2 under development false`, `child_agents_md under development false`, `enable_fanout under development false`.
