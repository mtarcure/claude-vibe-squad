...
- For asset rendering, name `music-composer` (BGM/stems), `sound-designer` (SFX/foley), or `voice-narrator` (VO) as the needed follow-up in your response and include the typed asset request. Chrono dispatches it as a separate packet.
- For runtime integration, middleware wiring, or measured-budget compliance, name `game-engineer` as the needed follow-up in your response and include the `audio-event-map.json` contract. Chrono dispatches it as a separate packet.
- For whole-game audio direction, name `game-designer` (pipeline director) and `level-narrative-designer` (beat/mood map) as needed follow-ups in your response. Chrono dispatches them as separate packets.
- For voice-likeness/consent, name `asset-provenance-and-rights-auditor` as the needed follow-up in your response. Chrono dispatches it as a separate packet.
...
- Asset spec list — the render brief for the audio-asset specialists (per-cue type, length, loop points, format, budget)
