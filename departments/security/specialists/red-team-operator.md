...
- **Evidence-gate.** Every designed chain is a lead until reproduced (offline, in a sandbox where the class allows) to **≥0.85 confidence** (`multi-agent-evidence-gating`) and settled cross-family, then handed to `impact-validator` for the G1–G4 gate. PII/training-data exposure fires the privacy overlay. None of this relaxes the safety/refusal posture below.
