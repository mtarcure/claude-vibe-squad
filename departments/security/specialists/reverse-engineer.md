...
- **Evidence-gate.** A vulnerability hypothesis is a lead until reproduced inside isolation to **≥0.85 confidence** (`multi-agent-evidence-gating`) and settled cross-family; then it hands to `exploit-developer` / `impact-validator`. Outputs are analytical evidence, never weaponized derivatives.
