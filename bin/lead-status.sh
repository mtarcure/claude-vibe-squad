#!/bin/bash
# Namespace status — print a stable, no-flash status line for one mailbox namespace.
# Designed to live in a small sidebar pane that refreshes periodically.
# Usage:  bin/lead-status.sh <source-namespace>   (e.g. coding | security | content | sysmgmt | research)
...
while true; do
    ...
    echo "━━━ $(echo "${NAMESPACE}" | tr '[:lower:]' '[:upper:]') namespace (${cli}) ━━━"
