...
NUDGE_RECEIPTS="${VAULT_ROOT}/scripts/python/nudge_receipts.py"
if [[ "${1:-}" == "--resend-due" ]]; then
    shift
    exec python3 "$NUDGE_RECEIPTS" resend-due \
        --state-dir "${VAULT_ROOT}/_state" \
...
printf '%s' "$msg" | python3 "$NUDGE_RECEIPTS" send \
    --state-dir "${VAULT_ROOT}/_state" \
    --registry "${VAULT_ROOT}/_state/active-tasks.json" \
    --task-id "$task_id" \
    --target-lane "$to_model" \
    --target "${SESSION}:${target_win}" \
