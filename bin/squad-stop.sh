...
if [[ ${chrono_responded} -eq 0 ]]; then
...

    {
        ...
        echo ""
        echo "Generated by \`bin/squad-stop.sh\` because Chrono did not respond to the shutdown request within 60s. This file is ignored runtime context, not durable product truth. Resume from \`_state/active-tasks.json\`, \`chrono/current.md\`, and each Lead's \`current.md\`."
        echo ""
        ...
        echo '```'
        echo ""
        echo "## Per-Lead current state"
        for lead in coding security content sysmgmt research; do
            echo ""
            echo "### ${lead}"
            echo '```markdown'
            cat "${VAULT_ROOT}/departments/${lead}/current.md" 2>/dev/null || echo "(missing)"
            echo '```'
        done
        echo ""
        ...
        for lead in coding security content sysmgmt research; do
            today=$(date +%Y-%m-%d)
            files=$(ls -1 "${VAULT_ROOT}/departments/${lead}/outbox/" 2>/dev/null | grep "${today}" || true)
            if [[ -n "${files}" ]]; then
                echo "- **${lead}**: ${files}"
            fi
        done
