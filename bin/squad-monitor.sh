...
#   2. Stale active  — Lead picked up a task but no response in >30m
#   3. Loop/thrash   — same Lead received >2 tasks in 30m (regen/SUPP thrash)
#
...
LEADS=(coding security content sysmgmt research)
...
# Hash last 50 lines of each lead pane. If hash is unchanged since last check
# AND the lead has unread inbox tasks → alert (once per stuck episode).

detect_stuck() {
    local lead="$1"
    local pane="${SESSION}:$(lead_window_name "${lead}")"
    local hash_file="${STATE_DIR}/${lead}-pane.hash"
    local ts_file="${STATE_DIR}/${lead}-pane.ts"
    local alerted_file="${STATE_DIR}/${lead}-stuck-alerted"

    # Capture pane (last 50 lines), compute hash
    local current_hash
    if $TEST_MODE && [[ "$lead" == "coding" ]]; then
    ...
    local inbox_tasks
    inbox_tasks=$(find "${VAULT_ROOT}/departments/${lead}/inbox" \
    ...
    send_alert "${lead} pane stuck >${stale_min}m with unread inbox (${task_name})"
    touch "${alerted_file}"
}

# ── DETECTOR 2: stale active ─────────────────────────────────────────────────
# Lead moved a task to active/ but hasn't responded in >30m.

detect_stale_active() {
    local lead="$1"
    local active_dir="${VAULT_ROOT}/departments/${lead}/active"
    ...
    while IFS= read -r task_file; do
        ...
        local alerted_file="${STATE_DIR}/${lead}-stale-${task_name}-alerted"
        [[ -f "$alerted_file" ]] && continue

        local age_min=$(( age / 60 ))
        send_alert "${lead} has stale active task (${task_name}, ${age_min}m old)"
        touch "${alerted_file}"
...
detect_thrash() {
    ...
    for lead in "${LEADS[@]}"; do
        # Collect body hashes for all tasks dispatched to this lead in window
        local hash_list=""
        while IFS= read -r task_id; do
            [[ -z "$task_id" ]] && continue
            local task_file=""
            for dir in inbox active archive; do
                local f="${VAULT_ROOT}/departments/${lead}/${dir}/${task_id}.md"
        ...
        local alerted_file="${STATE_DIR}/${lead}-thrash-${bucket}-alerted"
        [[ -f "$alerted_file" ]] && continue

        local window_min=$(( THRASH_WINDOW / 60 ))
        send_alert "${lead} received same task ${max_repeats}x in ${window_min}m — real thrash (duplicate body)"
        touch "${alerted_file}"
    done
}
...
auto_archive_completed() {
    local lead="$1"
    local active_dir="${VAULT_ROOT}/departments/${lead}/active"
    local outbox_dir="${VAULT_ROOT}/departments/${lead}/outbox"
    local archive_dir="${VAULT_ROOT}/departments/${lead}/archive"
    ...
    while IFS= read -r task_file; do
        ...
        echo "[$(date -u +%H:%M:%SZ)] AUTO-ARCHIVED: ${lead}/active/${task_name} (response exists, ${age}s old)"
        # Clear any stale-alerted flag for this task (it's resolved)
        rm -f "${STATE_DIR}/${lead}-stale-${task_name}-alerted"
    done < <(find "${active_dir}" -maxdepth 1 -name 'TASK-*.md' 2>/dev/null)
}
...
for lead in "${LEADS[@]}"; do
    detect_stuck         "$lead"
    detect_stale_active  "$lead"
    auto_archive_completed "$lead"
done
detect_thrash
