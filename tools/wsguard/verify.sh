...
bash "${HERE}/repro.sh" >"${HERE}/out/repro.log" 2>&1
TWIN="$(grep -c 'WARNING: write_scope path is gitignored' "${HERE}/out/repro.log")"
printf '+ grep -c "WARNING: write_scope path is gitignored" out/repro.log\n%s\n' "$TWIN"
[[ "$TWIN" == "0" ]] && ok=yes || ok=no
check "defective bash block emits no warning (detector is live)" "0 warnings from the twin" "$TWIN" "$ok"
