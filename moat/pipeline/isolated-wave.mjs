...
import { validateWaveResult } from "./wave-result.mjs";
...
// python program, and the wave is node. Callers may explicitly pass a reviewed
// replacement image; the default itself stays immutable and digest-pinned.
const DEFAULT_IMAGE = "nikolaik/python-nodejs@sha256:15a5d044754b9c8d7a86df9b7bb44087b8737070ea249af084d2670aeaad75ba";

const WAVE_ENVELOPE_SCHEMA = Object.freeze({
  type: "object",
  additionalProperties: false,
  required: ["waveResult", "evidence"],
  properties: {
    waveResult: { type: "object" },
    evidence: { type: "object" },
  },
});

async function parseWaveEnvelope(stdout) {
  let parsed;
  try {
  ...
  try {
    if (validateInstance(WAVE_ENVELOPE_SCHEMA, parsed).length > 0) return null;
    return (await validateWaveResult(parsed.waveResult)).length === 0 ? parsed : null;
  } catch {
    return null;
  }
}
...
export async function runIsolatedSyntheticWave({
  ...
  const isolatedRun = runnerFactory
    ? runnerFactory({ profile, canarySuite, approvedMountRoots: [MOAT_DIR] })
    : createIsolationRunner({
      runtime: createDockerRuntime(),
      profile,
      canarySuite,
      approvedMountRoots: [MOAT_DIR],
    });
  ...
  try {
    ...
    if (error?.code === "MOAT_ISOLATION_SPEC_REJECTED") {
      return {
        waveResult: null,
        isolation: { mode: "container", isolated: false, state: "isolation_spec_invalid" },
      };
    }
  ...
  const parsed = await parseWaveEnvelope(run.execution.stdout);
  if (!parsed) {
    return {
      waveResult: null,
