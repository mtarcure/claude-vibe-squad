...
def _firecrawl_zero_data_retention() -> bool:
    """Return whether the enterprise-only zero-retention option was explicitly enabled."""
    return os.environ.get("FIRECRAWL_ZERO_DATA_RETENTION", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _firecrawl_http_error(
    exc: httpx.HTTPStatusError, *, zero_data_retention: bool
) -> dict[str, Any]:
    """Normalize provider errors without silently weakening an explicit retention request."""
    if exc.response.status_code == 403 and zero_data_retention:
        return _err(
            "firecrawl_zero_data_retention_requires_enterprise",
            status_code=403,
...
def _firecrawl_ok(response: httpx.Response) -> dict[str, Any]:
    """Return a bounded success envelope that preserves the provider status receipt."""
...
def _firecrawl_json_request(path: str, payload: dict[str, Any], timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        return _firecrawl_ok(response)
    except httpx.HTTPStatusError as exc:
        return _firecrawl_http_error(
            exc,
            zero_data_retention=payload.get("zeroDataRetention") is True,
        )
...
def firecrawl_scrape(
    ...
    ``FIRECRAWL_ZERO_DATA_RETENTION=true`` opts into Firecrawl's enterprise-only
    zero-retention mode; it is omitted by default for plan compatibility.
    """
    ...
    if _firecrawl_zero_data_retention():
...
def firecrawl_crawl(
    ...
    Set ``FIRECRAWL_ZERO_DATA_RETENTION=true`` only on an enterprise plan.
    """
    payload = {
        "url": url,
...
def firecrawl_parse(
    ...
    zero_data_retention = _firecrawl_zero_data_retention()
    options = {
        "formats": _firecrawl_formats(formats),
        "onlyMainContent": bool(only_main_content),
    }
    ...
    try:
        ...
        return _firecrawl_http_error(exc, zero_data_retention=zero_data_retention)
    except Exception as exc:
        return _err(f"firecrawl_error: {type(exc).__name__}")
