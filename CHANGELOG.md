...
- **`bin/upgrade-specialists.py`** (789 lines) — automation script that injects v1.1 schema sections (Tools available, fan-out, escalate, do-not) across all 39 specialist files. Reads api-catalog for per-Lead-CLI tool pre-fill. Atomic write, idempotent, leaves `<FILL: ...>` placeholders for human-judgment fields. (commit `8149f60`)
