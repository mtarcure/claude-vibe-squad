---
name: loop-operator
description: "Run autonomous agent loops with explicit stop conditions, checkpoint progress, detect stalls, intervene safely when a loop fails to advance."
...
- `stall-detection`
- `safe-intervention`
- `bounded-autonomy-pattern` — define explicit stop conditions (iteration cap, time budget, success criteria) before any loop starts
- `stall-recovery-discipline` — graceful exit + state preservation when stall-detection fires, never just kill
