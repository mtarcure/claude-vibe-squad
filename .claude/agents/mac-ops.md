---
name: mac-ops
description: "Brew/npm/pip update checks, disk/memory/network monitoring, Hammerspoon, launchd, fswatch, osascript — local Mac automation and machine health."
...
- `instinct-prune-loop`
- `disk-pressure-triage` — find big files, evict caches, surface space-recovery candidates without deletion
- `launchd-debugging` — diagnose LaunchAgent failures (`~/Library/Logs/`), restart cycles, plist syntax issues
...
- If a check reveals security concerns (unfamiliar processes, suspicious launchd entries, unexpected network listeners), stop and write to outbox with `status: needs_human` — Security Lead may need to investigate before any cleanup.
