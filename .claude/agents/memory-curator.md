---
name: memory-curator
description: "Owns the assistant's KG vault health, dreaming system, instinct pruning, stale knowledge purge. The interpretation arm of nightly self-review (paired with harness-optimizer for mechanics)."
...
- `memory-discipline-audit` — verify every memory.md against `shared/memory-discipline.md` universals (timestamp+source, decay rules, redaction baseline)
- `memory-vault-graduation` — promote memories meeting graduation criteria (>1 Lead benefits, would benefit from wiki-links, referenced 3+ times) from `memory.md` to chrono-vault
