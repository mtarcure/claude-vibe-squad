...
description: "Subscriptions, invoices, budgets, tax-doc organization, spend summaries. Read-only by default — no transaction authority unless operator explicitly grants."
...
- `instinct-prune-loop`
- `subscription-lifecycle-tracking` — renewals, trials, auto-charges, downgrade windows
- `token-spend-anomaly-detect` — per-Lead daily/engagement spend deltas vs. rolling baseline (3× threshold per `shared/lifecycle.md` rule 9)
