...
**Files:** Create `plugins/chrono-vault/README.md` (detailed docs); Modify the **root `README.md`** — the "Tools, plugins & the optional daemon" section (~line 218) ALREADY catalogs the four chrono plugins, so **update the `chrono-vault` entry** there (it currently reads "KG + Obsidian + auto-capture") to describe the new memory system (private markdown vault + BM25 recall + write→recall→apply loop) and **link it to the plugin README** — describe the *capability*, never expose vault *content*; Modify `CLAUDE.md` (+ `chrono/CLAUDE.md`) memory/session-resume references; audit `docs/` for stale KG mentions. (Operator decisions 2026-07-14: update the existing plugins catalog entry + add the plugin README; do NOT add a separate memory section and do NOT extract chrono-vault into its own repo. Optional future: READMEs for the other plugins — chrono-content-engineer + chrono-research-arsenal lack them — but out of scope for this build.)
