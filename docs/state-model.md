...
These are runtime surfaces, not tracked content: a fresh public clone has none of them until the squad runs. Their absence is normal, not drift.

If these disagree with each other, treat it as drift. Run:
...
- `departments/*/outbox/`
- `departments/*/archive/`
- `_state/board-worktrees/`
- `_state/task-registry/`
- `_state/tmux-logs/`
- `_state/doctor-logs/`
- `_state/cleanup-logs/`

Public repos should not track mailbox contents. The launcher recreates missing mailbox directories on startup, and the board creates and removes a worktree per dispatch attempt under `_state/board-worktrees/`.
